...
class Handlers:
    ...
    @classmethod
    def check_remote_electrumsv_install(cls, url):
        """3 possibilities:
    ...
    @classmethod
    def check_remote_electrumsv_node_install(cls):
    ...
    def handle_remote_repo(cls, package_name, url, branch):
        ...
        if package_name == Config.ELECTRUMSV:
            cls.check_remote_electrumsv_install(url)

        if package_name == Config.ELECTRUMX:
            cls.check_remote_electrumx_install(url)

        if package_name == Config.ELECTRUMSV_NODE:
            cls.check_remote_electrumsv_node_install()
    ...
    def handle_local_repo(cls, package_name, path, branch):
        try:
            ...
            assert Path(
                path
            ).exists(), f"the path {path} to {package_name} does not exist!"
            if branch != "":
    ...
    def handle_electrumsv_args(cls, parsed_args):
        ...
        if parsed_args.repo == "":  # default
            ...
            cls.handle_remote_repo(
                Config.ELECTRUMSV, parsed_args.repo, parsed_args.branch
            )
    ...
    def handle_electrumx_args(cls, parsed_args):
        ...
        if parsed_args.repo == "":  # default
            ...
            cls.handle_remote_repo(
                Config.ELECTRUMX, parsed_args.repo, parsed_args.branch
            )
        elif parsed_args.repo.startswith("https://"):
            cls.handle_remote_repo(
                Config.ELECTRUMX, parsed_args.repo, parsed_args.branch
    ...
    def handle_electrumsv_node_args(cls, parsed_args):
        ...
        if parsed_args.repo == "":  # default
            ...
            cls.handle_remote_repo(
                Config.ELECTRUMSV_NODE, parsed_args.repo, parsed_args.branch
            )
        elif parsed_args.repo.startswith("https://"):
            cls.handle_remote_repo(
                Config.ELECTRUMSV_NODE, parsed_args.repo, parsed_args.branch
    ...
    def handle_electrumsv_indexer_args(cls, parsed_args):
        ...
        raise NotImplementedError(
            "electrumsv_indexer installation is not supported yet."
        )

        if parsed_args.repo == "":  # default
            parsed_args.repo = "????"
            cls.handle_remote_repo(
                Config.ELECTRUMSV_INDEXER, parsed_args.repo, parsed_args.branch
            )
        elif parsed_args.repo.startswith("https://"):
            cls.handle_remote_repo(
                Config.ELECTRUMSV_INDEXER, parsed_args.repo, parsed_args.branch
