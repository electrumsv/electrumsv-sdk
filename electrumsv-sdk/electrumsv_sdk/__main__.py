...
def main():
    ...
    """
    argparser = ArgParser()
    config: ImmutableConfig = argparser.manual_argparsing(sys.argv)
    argparser.validate_cli_args()

    app_state = AppState(config)
    app_state.handle_first_ever_run()

    # Call relevant entrypoint
    if config.namespace == NameSpace.INSTALL:
        app_state.controller.install(config)  # start() -> status_check() entrypoint of plugin

    if config.namespace == NameSpace.START:
        app_state.controller.start(config)  # start() -> status_check() entrypoint of plugin

    if config.namespace == NameSpace.STOP:
        app_state.controller.stop(config)  # -> stop() entrypoint of plugin

    if config.namespace == NameSpace.RESET:
        app_state.controller.reset(config)  # -> reset() entrypoint of plugin

    # Special built-in execution pathway (not part of plugin system)
    if config.namespace == NameSpace.NODE:
        app_state.controller.node(config)

    # Http 'GET' request to status_monitor (which itself is a plugin component of the SDK)
    if config.namespace == NameSpace.STATUS:
